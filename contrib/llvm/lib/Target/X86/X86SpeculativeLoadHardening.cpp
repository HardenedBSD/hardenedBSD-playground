...
STATISTIC(NumPUSHFPOPFSequences, "Number of pushf/popf sequences inserted");
STATISTIC(NumLAHFSAHFSequences, "Number of lahf/sahf sequences inserted");
...
static cl::opt<bool>
    EnablePostLoadHardening(PASS_KEY "-post-load",
...
static cl::opt<bool> EnablePostLoadHardeningWithLiveEFLAGS(
    PASS_KEY "-post-load-live-eflags",
    cl::desc("When hardening a value after it is loaded, do this even if "
             "EFLAGS are live by saving and restoring them."),
    cl::init(false), cl::Hidden);
...
static cl::opt<bool> EnableLAHF(
    PASS_KEY "-enable-lahf",
    cl::desc("Enable using the LAHF and SAHF instructions to load and store "
             "flags rather than using PUSHF and POPF when the target hardware "
             "supports it. Due to bugs with the use of these instructions this "
             "is currently disabled by default."),
    cl::init(false), cl::Hidden);

// Constant for the `OR*rr` instruction's EFLAG operand index.
static constexpr int OrInstEFLAGSOpIdx = 3;

// Constant for the `SHL*ri` instruction's EFLAG operand index.
static constexpr int ShiftInstEFLAGSOpIdx = 3;
...
private:
  ...
  void hardenLoadAddr(MachineInstr &MI, MachineOperand &BaseMO,
...
static MachineBasicBlock &splitEdge(MachineBasicBlock &MBB,
  ...
  DEBUG(dbgs() << "  Split edge from '" << MBB.getName() << "' to '"
               << Succ.getName() << "'.\n";);
  return NewMBB;
}

static void moveEdge(MachineBasicBlock &MBB, MachineBasicBlock &OldSucc,
                     int SuccCount, MachineBasicBlock &NewSucc,
                     MachineInstr *Br, MachineRegisterInfo &MRI,
                     const X86InstrInfo &TII, const TargetRegisterInfo &TRI) {
  assert(!OldSucc.isEHPad() && "Shouldn't get edges to EH pads!");

  MachineFunction &MF = *MBB.getParent();

  // If this is the last edge to the successor, we need to remove it from MBB.
  if (SuccCount == 1)
    MBB.replaceSuccessor(&OldSucc, &NewSucc);
  else
    MBB.splitSuccessor(&OldSucc, &NewSucc);

  // Update the branch instruction to point to the new successor.
  if (Br) {
    assert(Br->getOperand(0).getMBB() == &OldSucc &&
           "Didn't start with the right target!");
    Br->getOperand(0).setMBB(&NewSucc);
  } else if (!MBB.isLayoutSuccessor(&NewSucc)) {
    SmallVector<MachineOperand, 4> Cond;
    TII.insertBranch(MBB, &NewSucc, nullptr, Cond, DebugLoc());
  }

  // Adjust PHIs for new edge. This is more complex than when we first split
  // the edge.
  for (MachineInstr &MI : OldSucc) {
    if (!MI.isPHI())
      break;

    // First, find the incoming operands both for this edge and the old edge.
    int OldIdx = 0, NewSuccIdx = 0;
    for (int OpIdx = 1, NumOps = MI.getNumOperands(); OpIdx < NumOps;
         OpIdx += 2) {
      MachineOperand &OpMBB = MI.getOperand(OpIdx + 1);
      assert(OpMBB.isMBB() && "Block operand to a PHI is not a block!");
      if (OpMBB.getMBB() == &MBB) {
        assert(OldIdx == 0 && "Should only find this block once!");
        OldIdx = OpIdx;
      } else if (OpMBB.getMBB() == &NewSucc) {
        assert(NewSuccIdx == 0 && "Should only find the new successor once!");
        NewSuccIdx = OpIdx;
      }
      if (OldIdx && NewSuccIdx)
        break;
    }
    assert(OldIdx != 0 && "Must have an entry for the old edge!");
    assert(NewSuccIdx != 0 && "Must have an entry for the new successor!");

    MachineOperand &OrigIncomingV = MI.getOperand(OldIdx);
    MachineOperand &NewSuccIncomingV = MI.getOperand(NewSuccIdx);

    // Regardless of what path we take to update PHI entries, we'll need to
    // clear the old entry out if this is the last edge to the successor.
    auto RemoveOldOp = make_scope_exit([&] {
      if (SuccCount == 1) {
        MI.RemoveOperand(OldIdx + 1);
        MI.RemoveOperand(OldIdx);
      }
    });

    // If our original incoming value is the same as the value for the new
    // successor, we're done.
    if (OrigIncomingV.isIdenticalTo(NewSuccIncomingV)) {
      continue;
    }

    // Otherwise, if the new successor's incoming value is already a PHI in
    // that new successor, we just need to move our incoming value from this
    // PHI to the one in the new successor.
    if (NewSuccIncomingV.isReg() &&
        TRI.isVirtualRegister(NewSuccIncomingV.getReg()))
      if (auto *NewSuccIncomingMI =
              MRI.getUniqueVRegDef(NewSuccIncomingV.getReg())) {
        if (NewSuccIncomingMI->isPHI() &&
            NewSuccIncomingMI->getParent() == &NewSucc) {
          NewSuccIncomingMI->addOperand(MF, OrigIncomingV);
          NewSuccIncomingMI->addOperand(MF, MachineOperand::CreateMBB(&MBB));
          continue;
        }

        // There shouldn't be a PHI for an incoming value defined *within* the
        // new successor unless it is a PHI node, as we can only end up here
        // when it has been split out and only contains cmovs.
        assert(NewSuccIncomingMI->getParent() != &NewSucc &&
               "Unexpected def of incoming value!");
      }

    // Lastly, if the new successor's incoming vaule *isn't* yet a PHI, we need
    // to create a PHI in the new successor between our incoming value in its,
    // replace its incoming value in this PHI with the new PHI, and remove
    // ours.
    assert(MI.getDesc().getNumDefs() == 1 &&
           "Cannot have a multi-def PHI node!");
    unsigned PHIReg =
        MRI.createVirtualRegister(MRI.getRegClass(MI.defs().begin()->getReg()));
    auto NewPHIBuilder = BuildMI(NewSucc, NewSucc.begin(), DebugLoc(),
                                 TII.get(TargetOpcode::PHI), PHIReg);
    for (MachineBasicBlock *NewSuccPred : NewSucc.predecessors()) {
      NewPHIBuilder.add(NewSuccPred == &MBB ? OrigIncomingV : NewSuccIncomingV);
      NewPHIBuilder.addMBB(NewSuccPred);
    }
    NewSuccIncomingV.ChangeToRegister(PHIReg, /*isDef*/ false);
  }
}
...
bool X86SpeculativeLoadHardeningPass::runOnMachineFunction(
  ...
  // We support an alternative hardening technique this is trivial to
  // implement.
  if (HardenEdgesWithLFENCE) {
    ...
    for (MachineBasicBlock &MBB : MF) {
      ...
      if (std::next(MBB.getIterator()) != MF.end() &&
          MBB.isSuccessor(&*std::next(MBB.getIterator())))
        Blocks.insert(&*std::next(MBB.getIterator()));
      ...
      for (MachineInstr &MI : llvm::reverse(MBB)) {
        ...
        if (MI.getOpcode() == X86::JMP_1 ||
          ...
          Blocks.insert(MI.getOperand(0).getMBB());
      }
    }

    for (MachineBasicBlock *MBB : Blocks) {
      auto InsertPt = MBB->instr_begin();
      while (InsertPt != MBB->instr_end() && InsertPt->isPHI())
        ++InsertPt;
  ...
  auto EntryInsertPt = Entry.begin();
  ...
  if (HardenInterprocedurally && !FenceCallAndRet) {
    ...
    ZeroI->getOperand(1).setIsDead(true);
  ...
  // Hasher for sequences of condition codes.
  struct CheckedPathKeyInfo {
    using KeyT = std::pair<MachineBasicBlock *, std::vector<X86::CondCode>>;
    using MBBPtrInfoT = DenseMapInfo<MachineBasicBlock *>;

    static KeyT getEmptyKey() { return {MBBPtrInfoT::getEmptyKey(), {}}; }

    static KeyT getTombstoneKey() {
      return {MBBPtrInfoT::getTombstoneKey(), {}};
    }

    static unsigned getHashValue(const KeyT &Key) {
      return (unsigned)hash_combine(
    ...
    static bool isEqual(const KeyT &LHS, const KeyT &RHS) {
      assert(std::is_sorted(LHS.second.begin(), LHS.second.end()) &&
             "Codes not sorted!");
      assert(std::is_sorted(RHS.second.begin(), RHS.second.end()) &&
             "Codes not sorted!");
      return LHS == RHS;
    }
  };

  // Keep a map of a destination block and a sequence of condition codes to
  // a basic block that provides a checked path into the destination for the
  // particular flag sequence. This allows us to share the same checking
  // instructions whenever the flags entering the block match.
  DenseMap<std::pair<MachineBasicBlock *, std::vector<X86::CondCode>>,
           MachineBasicBlock *, CheckedPathKeyInfo>
      CheckedPaths;
  ...
  for (BlockCondInfo &Info : Infos) {
    ...
    DEBUG(dbgs() << "Tracing predicate through block: " << MBB.getName()
                 << "\n");
    ...
    auto BuildCheckingBlockForSuccAndConds =
        [&](MachineBasicBlock &MBB, MachineBasicBlock &Succ, int SuccCount,
            MachineInstr *Br, MachineInstr *&UncondBr,
            ArrayRef<X86::CondCode> Conds) -> MachineBasicBlock & {
      ...
      assert(InsertPt == CheckingMBB.end() ||
             !InsertPt->isPHI() &&
      ...
      for (X86::CondCode Cond : Conds) {
        ...
        auto CMovI = BuildMI(CheckingMBB, InsertPt, Loc, TII->get(CMovOp),
                         ...
                         .addReg(CurStateReg)
                         .addReg(PoisonReg);
        // If this is the last cmov and the EFLAGS weren't originally live-in,
        // mark them as killed.
        if (!LiveEFLAGS && Cond == Conds.back())
          CMovI->findRegisterUseOperand(X86::EFLAGS)->setIsKill(true);

        ++NumInstsInserted;
        DEBUG(dbgs() << "  Inserting cmov: "; CMovI->dump(); dbgs() << "\n");

        // The first one of the cmovs will be using the top level `PredStateReg`
        // and need to get rewritten into SSA form.
    ...
    for (auto *CondBr : CondBrs) {
      ...
      // See if we already have a checking version of this successor. If so, we
      // can just move the edge to point at it.
      MachineBasicBlock *&CheckingMBB = CheckedPaths[{&Succ, {InvCond}}];
      if (CheckingMBB) {
        moveEdge(MBB, Succ, SuccCount, *CheckingMBB, CondBr, *MRI, *TII, *TRI);
      } else {
        // Otherwise we need to build a checking version of the successor.
        CheckingMBB = &BuildCheckingBlockForSuccAndConds(
            MBB, Succ, SuccCount, CondBr, UncondBr, {InvCond});
      }
    ...
    // Sort and unique the codes to canonicalize them.
    std::sort(UncondCodeSeq.begin(), UncondCodeSeq.end());
    UncondCodeSeq.erase(std::unique(UncondCodeSeq.begin(), UncondCodeSeq.end()),
                        UncondCodeSeq.end());

    MachineBasicBlock *&CheckingMBB = CheckedPaths[{UncondSucc, UncondCodeSeq}];
    if (CheckingMBB) {
      moveEdge(MBB, *UncondSucc, /*SuccCount*/ 1, *CheckingMBB, UncondBr, *MRI,
               *TII, *TRI);
      continue;
    }
    ...
    // Otherwise we need to build a checking version of the successor.
    CheckingMBB = &BuildCheckingBlockForSuccAndConds(
        MBB, *UncondSucc, /*SuccCount*/ 1, UncondBr, UncondBr, UncondCodeSeq);
  }
  ...
  DEBUG(dbgs() << "Final speculative load hardened function:\n"; MF.dump();
        dbgs() << "\n"; MF.verify(this));
  return true;
}
...
X86SpeculativeLoadHardeningPass::collectBlockCondInfo(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    if (Info.CondBrs.empty()) {
      ++NumBranchesUntraced;
      DEBUG(dbgs() << "WARNING: unable to secure successors of block:\n";
            MBB.dump());
      continue;
    }
...
static bool isDataInvariantLoad(MachineInstr &MI) {
  ...
  case X86::CVTSS2SDrm_Int:
    // We don't handle loads into FP registers yet.
    return false;
    ...
    // time operations so we call them out here separatel.
  case X86::IMUL16rm:
  case X86::IMUL16rmi8:
  case X86::IMUL16rmi:
  case X86::IMUL32rm:
  ...
  case X86::BEXTRI64mi:
    ...
    if (!MI.findRegisterDefOperand(X86::EFLAGS)->isDead()) {
      ...
      DEBUG(dbgs() << "    Unable to harden post-load due to EFLAGS: ";
            MI.dump(); dbgs() << "\n");
      return false;
    }
...
static bool isEFLAGSLive(MachineBasicBlock &MBB, MachineBasicBlock::iterator I,
                         const TargetRegisterInfo &TRI) {
  // Check if EFLAGS are alive by seing if there is a def of them or they
...
void X86SpeculativeLoadHardeningPass::checkAllLoads(
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    for (MachineInstr &MI : MBB) {
      ...
      if (MemRefBeginIdx < 0) {
        DEBUG(dbgs() << "WARNING: unable to harden loading instruction: ";
              MI.dump());
        continue;
      }
      ...
      if (EnablePostLoadHardening && isDataInvariantLoad(MI) &&
          (EnablePostLoadHardeningWithLiveEFLAGS ||
           !isEFLAGSLive(MBB, std::next(MI.getIterator()), *TRI)) &&
      ...
      // Record this instruction for address hardening and record its regitser
...
unsigned X86SpeculativeLoadHardeningPass::saveEFLAGS(
  ...
  unsigned OFReg = 0;
  MachineFunction &MF = *MBB.getParent();
  // If we can save flags in registers rather than on the stack, try to
  // do so. FIXME: LAHF/SAHF pattern crashes register pressure synthesis.
  if (EnableLAHF && Subtarget->hasLAHFSAHF()) {
    // Save OF and the other flags to registers so we can restore them.
    OFReg = MRI->createVirtualRegister(&X86::GR8RegClass);
    BuildMI(MBB, InsertPt, Loc, TII->get(X86::SETOr), OFReg);
    ++NumInstsInserted;
    // FIXME: How do we know whether AH is already live?
    BuildMI(MBB, InsertPt, Loc, TII->get(X86::LAHF));
    ++NumInstsInserted;
    ++NumLAHFSAHFSequences;
  } else {
    // If we end up needing to preserve EFLAGS we'll need to mark the
    // function as implicitly containing stack adjustments so that the
    // push/pop sequence don't clobber something at the start of the
    // redzone, and mark the stack adjustment as opaque so we don't try
    // to use the stack pointer to index locals and register spills
    // within the pushf/popf range.
    MF.getFrameInfo().setHasCopyImplyingStackAdjustment(true);
    MF.getFrameInfo().setHasOpaqueSPAdjustment(true);
    BuildMI(MBB, InsertPt, Loc, TII->get(X86::PUSHF64));
    ++NumInstsInserted;
    ++NumPUSHFPOPFSequences;
  }
  return OFReg;
}
...
void X86SpeculativeLoadHardeningPass::restoreEFLAGS(
    ...
    unsigned OFReg) {
  if (EnableLAHF && Subtarget->hasLAHFSAHF()) {
    BuildMI(MBB, InsertPt, Loc, TII->get(X86::ADD8ri),
            MRI->createVirtualRegister(&X86::GR8RegClass))
        .addReg(OFReg)
        .addImm(127);
    ++NumInstsInserted;
    BuildMI(MBB, InsertPt, Loc, TII->get(X86::SAHF));
    ++NumInstsInserted;
  } else {
    BuildMI(MBB, InsertPt, Loc, TII->get(X86::POPF64));
    ++NumInstsInserted;
  }
}
...
void X86SpeculativeLoadHardeningPass::mergePredStateIntoSP(
  ...
  auto ShiftI = BuildMI(MBB, InsertPt, Loc, TII->get(X86::SHL64ri), TmpReg)
                    ...
                    .addImm(47);
  ShiftI->getOperand(ShiftInstEFLAGSOpIdx).setIsDead(true);
  ++NumInstsInserted;
...
unsigned X86SpeculativeLoadHardeningPass::extractPredStateFromSP(
  ...
  BuildMI(MBB, InsertPt, Loc, TII->get(TargetOpcode::COPY), TmpReg).addReg(X86::RSP);
  auto ShiftI =
...
void X86SpeculativeLoadHardeningPass::hardenLoadAddr(
  ...
  if (BaseMO.isFI()) {
    ...
    DEBUG(dbgs() << "  Skipping hardening base of explicit stack frame load: ";
          MI.dump(); dbgs() << "\n");
    ...
    DEBUG(dbgs() << "  Cannot harden base of "
  ...
  // If EFLAGS is live, defined by an instruction and that instruction
  // doesn't itself read EFLAGS, see if we can move the insert point up
  // to there, as EFLAGS cannot be live at that point.
  auto InsertPt = MI.getIterator();
#if 0
  if (EFLAGSLive && EFLAGSDefInstr &&
      !EFLAGSDefInstr->readsRegister(X86::EFLAGS) &&
      llvm::all_of(
          llvm::make_range(EFLAGSDefInstr->getIterator(), MI.getIterator()),
          [&](MachineInstr &MI) {
            if (MI.definesRegister(StateReg, TRI))
              return false;
            for (MachineOperand *Op : HardenOpRegs)
              if (MI.definesRegister(Op->getReg(), TRI))
                return false;

            return true;
          })) {
    InsertPt = EFLAGSDefInstr->getIterator();
    EFLAGSLive = false;
  }
#endif
  ...
  bool RestoreEFLAGS = false;
  unsigned OFReg;
  if (EFLAGSLive && !Subtarget->hasBMI2()) {
    RestoreEFLAGS = true;
    EFLAGSLive = false;

    OFReg = saveEFLAGS(MBB, InsertPt, Loc);
  }
  ...
  for (MachineOperand *Op : HardenOpRegs) {
    ...
    if (!EFLAGSLive) {
      ...
      OrI->getOperand(OrInstEFLAGSOpIdx).setIsDead(true);
      ++NumInstsInserted;
      DEBUG(dbgs() << "  Inserting or: "; OrI->dump(); dbgs() << "\n");
    } else {
      ...
      (void)ShiftI;
      ++NumInstsInserted;
      DEBUG(dbgs() << "  Inserting shrx: "; ShiftI->dump(); dbgs() << "\n");
    }
  ...
  // And restore the flags if needed, from registers or the stack.
  if (RestoreEFLAGS)
    restoreEFLAGS(MBB, InsertPt, Loc, OFReg);
}

MachineInstr *X86SpeculativeLoadHardeningPass::sinkPostLoadHardenedInst(
    MachineInstr &InitialMI,
    SmallPtrSetImpl<MachineInstr *> &HardenedLoads) {
  assert(isDataInvariantLoad(InitialMI) && "Cannot get here with a non-invariant load!");

  // See if we can sink hardening the loaded value.
  auto SinkCheckToSingleUse = [&](MachineInstr &MI) -> Optional<MachineInstr *> {
    ...
    for (MachineInstr &UseMI : MRI->use_instructions(DefReg)) {
      ...
      if (!isDataInvariant(UseMI) || UseMI.getParent() != MI.getParent() ||
          (EnablePostLoadHardeningWithLiveEFLAGS ||
           isEFLAGSLive(*UseMI.getParent(), std::next(UseMI.getIterator()),
                        *TRI)))
        return {};
...
void X86SpeculativeLoadHardeningPass::hardenPostLoad(
  ...
  assert(isDataInvariantLoad(MI) && "Cannot get here with a non-invariant load!");
  ...
  if (!EFLAGSLive) {
    ...
    auto Shift1I = BuildMI(MBB, InsertPt, Loc, TII->get(ShiftOpCode), ShiftedReg)
                   .addReg(StateReg)
                   .addReg(ShiftInputReg);
    ++NumInstsInserted;
    DEBUG(dbgs() << "  Inserting shrx: "; Shift1I->dump(); dbgs() << "\n");
    ...
    if (DefRegBytes == ShiftRegBytes) {
      ...
      auto Shift2I =
          BuildMI(MBB, InsertPt, Loc, TII->get(ShiftOpCode), OldDefReg)
              ...
              .addReg(ShiftInputReg);
      ++NumInstsInserted;
      DEBUG(dbgs() << "  Inserting shrx: "; Shift2I->dump(); dbgs() << "\n");
    } else {
      ...
      BuildMI(MBB, InsertPt, Loc, TII->get(TargetOpcode::COPY),
              OldDefReg)
...
void X86SpeculativeLoadHardeningPass::checkReturnInstr(
  ...
  mergePredStateIntoSP(MBB, InsertPt, Loc, PredStateSSA.GetValueAtEndOfBlock(&MBB));
}
